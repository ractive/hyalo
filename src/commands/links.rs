...
/// List outgoing links from a single file.
pub fn links(dir: &Path, file: &str, format: Format) -> Result<CommandOutcome> {
...
/// List unresolved links from a single file.
pub fn unresolved(dir: &Path, file: &str, format: Format) -> Result<CommandOutcome> {
    let (full_path, rel_path) = match discovery::resolve_file(dir, file) {
        Ok(r) => r,
        Err(e) => return Ok(resolve_error_to_outcome(e, format)),
    };
...
mod tests {
    ...
    fn links_single_file() {
        let tmp = setup_vault();
        let out = unwrap_success(links(tmp.path(), "note-a.md", Format::Json).unwrap());
    ...
    fn links_path_populated() {
        let tmp = setup_vault();
        let out = unwrap_success(links(tmp.path(), "note-b.md", Format::Json).unwrap());
        let parsed: serde_json::Value = serde_json::from_str(&out).unwrap();
        let links_arr = parsed["links"].as_array().unwrap();
    ...
    fn unresolved_single_file() {
        let tmp = setup_vault();
        let out = unwrap_success(unresolved(tmp.path(), "note-a.md", Format::Json).unwrap());
        let parsed: serde_json::Value = serde_json::from_str(&out).unwrap();
        let links_arr = parsed["links"].as_array().unwrap();
    ...
    fn links_file_not_found() {
        let tmp = setup_vault();
        let result = links(tmp.path(), "nope.md", Format::Json).unwrap();
        match result {
    ...
    fn links_empty_file() {
        ...
        let out = unwrap_success(links(tmp.path(), "empty.md", Format::Json).unwrap());
        let parsed: serde_json::Value = serde_json::from_str(&out).unwrap();
