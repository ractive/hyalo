...
- **Detect broken heading anchors**: `hyalo find --broken-links` reports a `[[Foo#Section]]` / `[t](foo.md#Section)` whose target file exists but whose `#Section` heading does not, as a `broken_anchor` category distinct from a broken target (never both on one link). Case-insensitive exact match; `^block-id` refs skipped. Rebuild the index (`hyalo create-index`) after upgrading to pick up anchor data on the `--index` path.
