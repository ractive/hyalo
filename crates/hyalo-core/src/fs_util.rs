...
/// Source-compatible guard retained for callers of the pre-0.24 bulk-write API.
///
/// Mutation durability and progress now belong to explicit rooted
/// `WriteSession`s. This inert guard deliberately carries no process-global
/// state; dropping it has no side effects.
#[deprecated(note = "use an explicit rooted::WriteSession")]
#[must_use = "retain the guard for source compatibility with older callers"]
pub struct WritePhase;

#[allow(deprecated)]
impl WritePhase {
    /// Begin an inert compatibility phase.
    pub fn begin(_total: usize, _label: &'static str) -> Self {
        Self
    }
}
...
#[allow(dead_code)] // private legacy adapter retained for its focused compatibility tests
pub fn atomic_write(path: &Path, data: &[u8]) -> Result<()> {
    write_impl(None, path, data)
}

#[cfg(test)]
mod write_phase_compatibility_tests {
    #[test]
    #[allow(deprecated)]
    fn published_write_phase_begin_remains_source_compatible_and_inert() {
        let _phase: super::WritePhase = super::WritePhase::begin(2, "compatibility");
    }
}
...
fn write_impl(vault_root: Option<&Path>, path: &Path, data: &[u8]) -> Result<()> {
    let parent = write_no_dir_sync(vault_root, path, data)?;
...
///
/// Returns the parent so the caller can perform the publication's explicit
/// directory durability step after the replacement has landed.
fn write_no_dir_sync(vault_root: Option<&Path>, path: &Path, data: &[u8]) -> Result<PathBuf> {
    let dest = resolve_write_target(path)?;
