...
/// Result of a partial-failure batch write: per-file outcomes plus a convenience
/// flag indicating whether any plan failed.
#[derive(Debug, Clone, Serialize, Default)]
pub struct PartialExecuteReport {
    /// One outcome per input plan, in input order.
    pub outcomes: Vec<PlanOutcome>,
}

impl PartialExecuteReport {
...
fn execute_plans_with_session(
    ...
    let results = write_plans_in_parallel(&canonical_vault, plans, receipt_paths, &session);
...
mod tests {
    ...
    // ---------------------------------------------------------------------------
    // execute_plans_partial (L-11)
    // ---------------------------------------------------------------------------

    fn simple_plan(dir: &Path, rel: &str, new_content: &str) -> RewritePlan {
        RewritePlan {
            path: dir.join(rel),
    ...
    #[test]
    fn execute_plans_partial_all_succeed() {
        let vault = create_vault(&[("a.md", "old-a\n"), ("b.md", "old-b\n")]);
        let plans = vec![
            simple_plan(vault.path(), "a.md", "new-a\n"),
            simple_plan(vault.path(), "b.md", "new-b\n"),
        ];
        let report = execute_plans_partial(vault.path(), &plans).unwrap();
        assert!(!report.has_failures());
        assert_eq!(report.applied_paths(), vec!["a.md", "b.md"]);
        assert_eq!(
    ...
    fn bulk_finalization_failure_retains_commits_receipts_and_source_conflicts() {
        ...
        let receipt_paths: Vec<_> = plans.iter().map(|plan| plan.rel_path.clone()).collect();
        let execution = execute_plans_with_session(
            vault.path(),
            &plans,
            &receipt_paths,
        ...
        assert_eq!(execution.receipts.len(), 8);
        for outcome in execution.report.outcomes {
    ...
    #[cfg(unix)]
    #[test]
    fn execute_plans_partial_continues_past_failure() {
        // L-11: a mid-batch write failure records the failed file but the
        // remaining files still get written; the report is honest about both.
        use std::os::unix::fs::PermissionsExt;

        let vault = create_vault(&[("a.md", "old-a\n"), ("b.md", "old-b\n")]);
        // Make a.md's parent unwritable via a read-only subdir so its write fails
        // while b.md (at the vault root) still succeeds.
        fs::create_dir(vault.path().join("locked")).unwrap();
        fs::write(vault.path().join("locked/a.md"), "old-a\n").unwrap();
        let mut perms = fs::metadata(vault.path().join("locked"))
            .unwrap()
            .permissions();
        perms.set_mode(0o555);
        fs::set_permissions(vault.path().join("locked"), perms).unwrap();

        let plans = vec![
            simple_plan(vault.path(), "locked/a.md", "new-a\n"),
            simple_plan(vault.path(), "b.md", "new-b\n"),
        ];
        let report = execute_plans_partial(vault.path(), &plans).unwrap();

        // Restore perms for cleanup.
        let mut restore = fs::metadata(vault.path().join("locked"))
            .unwrap()
            .permissions();
        restore.set_mode(0o755);
        let _ = fs::set_permissions(vault.path().join("locked"), restore);

        assert!(report.has_failures());
        assert_eq!(report.outcomes.len(), 2);
        assert!(!report.outcomes[0].applied, "locked/a.md write should fail");
        assert!(report.outcomes[0].error.is_some());
        assert!(
            report.outcomes[1].applied,
            "b.md write should still succeed"
        );
        assert_eq!(report.applied_paths(), vec!["b.md"]);
        assert_eq!(
            fs::read_to_string(vault.path().join("b.md")).unwrap(),
            "new-b\n"
        );
