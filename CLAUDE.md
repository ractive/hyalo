...
- **Inspect config**: `hyalo config` (text) or `hyalo config --format json` — shows effective dir, config path, hints, format, site_prefix. JSON uses the standard envelope, so `hyalo config --jq '.results.dir'` works
