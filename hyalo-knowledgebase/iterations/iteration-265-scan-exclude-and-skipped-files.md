...
- [x] DEC-277 (tentative): a new `[scan]` section in `.hyalo.toml` with
...
- [x] hyalo-core discovery: apply the globs once in the walker, reusing the
...
- [x] `hyalo config` text and JSON print the effective `[scan] exclude`
      (`results.scan.exclude`, empty list by default).
- [x] Unit tests on the walker; e2e in `crates/hyalo-cli/tests/e2e`: a vault
...
- [x] Docs: `hyalo config -h/--help`, the `.hyalo.toml` reference page under
...
- [x] DEC-278 (tentative): per-file YAML diagnostics are collected, not
...
- [x] hyalo-core: route the parse failure through a `SkippedFile { path,
...
- [x] `summary`: add `results.files.skipped` (unparsable), `results.files.
...
- [x] `create-index` full build: exclude invalid-UTF-8 files from BM25
      ...
      `Files:`.
- [x] Parity test: build an index over a fixture containing one invalid-UTF-8
...
- [x] Enumerate which commands check `.hyalo-index` staleness today
...
- [x] DEC-280 (tentative): one policy. Proposal: every `--index` command does
...
- [x] Implement the policy in hyalo-core index loading; e2e: `find --index
...
- [x] Docs: `--index` long help paragraph shared by all commands, skill file,
      changelog.

### CONFIG-1: malformed `.hyalo.toml` is loud (BUG-19)

- [x] DEC-279 (tentative): today a `.hyalo.toml` that fails to parse blocks
      ...
      "gates".
- [x] Implement in hyalo-cli config loading; e2e: a `.hyalo.toml` with an
      unknown top-level key makes `hyalo lint` exit 1 with the parse error and
      `hyalo find --count` still answer with the warning.
- [x] Docs: `hyalo config --help` malformed paragraph, `.claude/CLAUDE.md`
      config-discovery bullet, skill file, changelog.

## Acceptance criteria

- [x] kepano-obsidian, cwd `../kepano-obsidian`, clean checkout, no config
      ...
      wc -l` → 0.
- [x] `../kepano-obsidian` with a scratch `.hyalo.toml` containing
...
- [x] Changelog entries via `hyalo changelog add`; DEC-277 through DEC-280
