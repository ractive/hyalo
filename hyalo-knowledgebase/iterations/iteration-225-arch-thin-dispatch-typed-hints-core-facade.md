...
- [x] ARCH-1: extract one handler per command
...
- [x] ARCH-4: stop building hints as hand-written strings. Introduce a
      ...
      reach
- [x] ARCH-5: curate a root `pub use` façade in `hyalo-core/lib.rs`
      ...
      consumers appear
- [x] Prove the ARCH-1 win: at least one command's warning/policy behavior
      gains an in-process unit test that previously required an e2e spawn
- [x] Docs: decision-log entries for the handler pattern, the hint API, and
      the core façade boundary; update any contributor notes on "where does a
      new command go"

## Acceptance criteria

- [x] `dispatch.rs` arms no longer contain command business logic — each arm
      parses args and calls a `commands::<cmd>::run`; the god-function line
      count drops materially (record before/after)
- [x] New hints cannot be written as raw strings without going through the
      argv-based `HintBuilder`; the migrated hints still pass
      `hint_execution.rs`
- [x] `hyalo-core`'s public surface is a curated façade; plumbing modules are
      `pub(crate)`; the workspace still builds and all tests pass
- [x] At least one previously-e2e-only behavior is now covered by an
      in-process unit test

## Results (2026-08-25)

- `dispatch.rs`: **3024 → 876 lines**; every arm with business logic now
  forwards to `commands::<cmd>::run` (find, lint, task, views, mv, set,
  remove, append, read, properties, tags, summary, backlinks, links,
  changelog, okf, madr, types, lint-rules). Pure structural move — e2e suite
  (1737 tests) green before and after.
- ARCH-4: `hints::HintBuilder` (argv + `shell_quote` + `argv()` for clap
  round-trip); all `build_command_*` builders, config hints and
  `profile_lint_hint` migrated; guard test `no_raw_hyalo_command_literals`
  fails the suite on new raw `"hyalo …"` literals; `hint_builder_commands_parse`
  feeds builder output to the real parser in-process.
- ARCH-5: `util`, `common_words`, `case_index`, `fs_util` are `pub(crate)`
  with curated root re-exports; supported surface documented in crate root.
- ARCH-1 proof: `find::run::strict_exit_code` unit test covers the
  previously e2e-only `--strict` exit-code policy.
- Docs: DEC-225 in decision-log.md.
