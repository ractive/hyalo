...
- [x] DEC-241 — stale-index detection for `--index` mutations: mtime-check
      indexed entries before the `links fix` / `links auto` discovery pass,
      rescan drift from disk, warn; record the decision in
      [[decision-log]]
- [x] UX-1 — every "run `hyalo find --file <glob>`" hint says `--glob`
      instead (`--file` does not glob)
- [x] UX-2 — `--iteration` also matches zero-padded IDs and files in
      subdirectories of the template's directory (`--iteration 2` reaches
      `iterations/done/iteration-02-links.md`)
- [x] UX-4 — lint sorts error-carrying files first so a display cap can
...
- [x] E2e tests for every changed command surface (`links fix`,
...
- [x] BUG-2 repro from the dogfood report: append a broken `[[…]]` to an
...
- [x] `hyalo set nonexistent.md …` hint suggests `find --glob`, and the
      suggested command actually works for a glob
- [x] `hyalo read --iteration 2` resolves `iterations/done/iteration-02-*.md`
      in this vault; `find --iteration 2` lists it
- [x] A vault with 4 errors in files ranked below 50 shows them in the
