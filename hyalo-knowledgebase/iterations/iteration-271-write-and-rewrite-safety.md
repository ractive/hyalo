...
- **Superseded design:** `--from X --to X` is a no-op that says so;
      `--to <existing>` still reports `conflicts`
...
- **Transferred to iteration 272:** `new_target` and text-mode output in `--dry-run`
      equal the replacement `--apply` writes,
      ...
      **Deferred — see Outcome.** Dry-run and apply share `build_replacements_for_file`, so they
      ...
      not hold at this iteration's close. Subsequently resolved in
      [[iterations/iteration-272-resolution-completeness]] Part F with additive
      `emitted_target`; `new_target` retains its vault-relative meaning. Fuzzy-bucket
      coverage followed in [[iterations/iteration-277-link-graph-parity-and-write-performance]]
      FIX-1. The original `new_target` string-equality design was not implemented.
...
- **Partially met here; reporting transferred:** BUG-4: on the mdn css copy `--apply`
      yields case-only diffs or none; every dry-run
      ...
      the CASE-2 reporting change, resolved through `emitted_target` in
      [[iterations/iteration-272-resolution-completeness]] and completed for fuzzy plans in
      [[iterations/iteration-277-link-graph-parity-and-write-performance]], as recorded above.
