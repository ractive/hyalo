...
- [ ] Introduce one shared constant (e.g. `LIST_COMMANDS` in `output.rs` or
...
- [ ] Verify `views list` and `types list` belong in that constant — both
      currently return a `total` and accept `--count` (7 and 6), while the
      error message claims they do not.
- [ ] Add the 8 missing commands to COMMAND REFERENCE: `changelog`, `config`,
...
- [ ] Fix the global-flags reference block: `--format json|text|github`, the
      TTY-dependent default (not "default: json"), and add `--index-file`.
- [ ] Fix `hints.rs:1439` — `["tags", "summary", "--limit", "0"]`.
- [ ] Add the missing "show all N properties" hint to `properties summary`,
      mirroring the tags one, so the symmetric commands behave symmetrically.
- [ ] Replace the substring-based hint assertions with an **execution-based**
      ...
      is the model.
- [ ] Wrap `hyalo config` in the standard envelope (`results` / `hints`),
      ...
      ignoring the filter.
- [ ] Add `list` as an alias for `summary` on `properties`/`tags`, and
      `summary` as an alias for `list` on `types`/`views`/`lint-rules`, so
      the verb a user learned in one group works in the others.
- [ ] Document the positional form for `read` and `backlinks` in COMMAND
      ...
      `-f/--file`.
- [ ] Decide `mv`'s dual default (single-file writes immediately; batch
...
- [ ] `LIST_COMMANDS` (or equivalent) is the single source for all five
      enumerations — `grep` finds no second hand-written list
- [ ] xtask gate fails when a clap subcommand is missing from COMMAND
      REFERENCE — proven by temporarily removing one entry
- [ ] execution-based hint gate runs every emitted hint and fails on a
      non-zero exit — proven by temporarily reverting the `hints.rs:1439` fix
- [ ] `hyalo tags summary` hint runs clean; `hyalo properties summary` emits
      an equivalent working hint
- [ ] `hyalo config --format json` carries `results` and an array `hints`;
      `hyalo config --jq '.results.dir'` prints the dir
- [ ] `hyalo tags list`, `hyalo types summary`, `hyalo views summary`,
      `hyalo lint-rules summary` all exit 0
- [ ] `hyalo mv --file a.md --to b.md --apply` errors with the guidance
      message — test name `mv_single_file_rejects_apply`
- [ ] no phrase in `stale-help-patterns.toml` reappears; `cargo fmt`,
