...
- [x] F3-1 (HIGH): bound `--jq` evaluation. (a) run the jaq iterator on a
...
- [x] F3-2: fix `upsert_managed_section` to anchor the END marker strictly
...
- [x] M-1: catch the UTF-8 (and read) error per file in the lint loop, emit
...
- [x] L-1: route `write_snapshot` through `fs_util::atomic_write`/
...
- [x] M-2: on Windows, reject any rel path with a `Prefix` component or a
...
- [x] ADVISORY-a: triage `anyhow` RUSTSEC-2026-0190 — bump anyhow if a fixed
...
- [x] ADVISORY-b: map the saphyr `Anchors {…}` (and any sibling budget
...
- [x] ADVISORY-c: move the case-insensitivity probe to a temp dir on the
...
- [x] `--jq 'def f: f; f'` and `--jq '[range(3e8)] | length'` both error
      within a bounded time/memory instead of hanging or OOMing; the limits
      are documented in `--jq --help`
- [x] The F3-2 stray-marker repro round-trips through `init --claude` /
      `deinit` with exactly one managed section and no orphan
- [x] One invalid-UTF-8 file no longer aborts `lint` or `lint --fix`; the
      rest of the vault is linted/fixed and the bad file is reported once
- [x] `create-index -o` onto a symlink follows one consistent, tested policy
      shared with the frontmatter write path (or DEC-062 is amended)
- [x] Windows drive-relative and ADS rel paths are rejected by both gates
      (tested under `#[cfg(windows)]`)
- [x] No user-facing error contains a saphyr/parser-internal debug struct
- [x] `cargo deny check` is green with a documented disposition for
      RUSTSEC-2026-0190
