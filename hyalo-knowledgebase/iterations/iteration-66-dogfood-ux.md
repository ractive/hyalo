...
- [ ] Add tip to `Links` command help pointing to `summary` and `find --broken-links`

### UX-2: `--reverse` flag for sort

- [ ] Add `#[arg(long)] reverse: bool` to `Find` variant in main.rs
- [ ] Thread `reverse` param through to `find()` and `find_from_index()` in find.rs
- [ ] After `apply_sort()`, call `results.reverse()` when flag is set
- [ ] Disable short-circuit optimization when `--reverse` is active
- [ ] Add e2e tests: `--sort title --reverse`, `--reverse --limit`, `--reverse` alone

### UX-1: `--title` filter flag

- [ ] Add `#[arg(long)] title: Option<String>` to `Find` variant in main.rs
- [ ] Force `fields.title = true` when `--title` is given (same pattern as `--broken-links`)
- [ ] After outline sections computed (line 242), extract title and match against pattern
- [ ] Pattern matching: case-insensitive substring by default, `~=` prefix for regex
- [ ] `continue` on mismatch (same pattern as property filters at line 235)
- [ ] Add e2e tests: H1 match, case-insensitive, regex mode, frontmatter title match

### UX-3: `--ignore-target` for `links fix`

- [ ] Add `#[arg(long)] ignore_target: Vec<String>` to `LinksAction::Fix` in main.rs
- [ ] Filter broken links before passing to `LinkMatcher::plan_fixes()` in links.rs
- [ ] Report ignored count in output: `"ignored": N`
- [ ] Add e2e tests: substring match, multiple patterns (OR), Hugo template skip
...
- [ ] Manual smoke tests on both doc directories
