...
- [ ] Inventory every executable leaf: target cardinality, output/count/projection capabilities,
...
- [ ] Introduce private-field PreparedInvocation, OutputPlan and distinct
...
- [ ] Reject unsupported count/format/projection combinations and compile jq syntax before any
...
- [ ] Normalize IndexIntent::Read/Create/Drop and resolve global/local index destination aliases
...
- [ ] Resolve files-from once and enforce single-target cardinality before opening content.
...
- [ ] Migrate read, backlinks, task read and mutation output preflight. Remove their old
...
- [ ] Capture observable CLI contracts on deterministic fixtures: stdout bytes, stderr, exit
...
- [ ] Produce a test-consumable descriptor from Clap plus the exhaustive capability metadata,
...
- [ ] Define exact single-input outcomes: absent required target becomes a Clap usage error
...
- [ ] Produce effective query inputs (filters, section/language policy and selection provenance)
...
- [ ] Invalid --count and malformed --jq on set/append/task leave all fixture bytes unchanged;
  no index/config write occurs.

- [ ] Repeated named files for single-file commands produce a structured cardinality error even
  when the second path is missing; zero/one/many selections are tested.

- [ ] CLI/config/default precedence, -- terminators, --flag=value, TTY/default formats and
  existing TS argument declarations remain compatible except documented corrections.

- [ ] Actual runtime dispatch accepts only the prepared output/input path for migrated commands;
...
- [ ] Add a small rooted-filesystem module inside hyalo-core. Distinguish VaultRoot,
...
- [ ] Keep directory-entry identity separate from followed-file identity so alias/referent moves
...
- [ ] Provide confined open, prepare replacement, commit replacement, no-replace move and
...
- [ ] Encapsulate platform behavior behind concrete small helpers. Verify Unix/macOS and Windows
...
- [ ] Introduce for migrated consumers, alongside a temporarily retained legacy WritePhase, an
...
- [ ] Migrate one real config replacement and indexed regex read; retain safe compatibility
...
- [ ] External file and parent-directory symlinks are rejected before read/write/delete in
  migrated operations; valid in-root aliases retain their documented semantics.

- [ ] Existing destination is never overwritten by the no-replace primitive, including a
  destination introduced between planning and execution.

- [ ] Failure before persistence preserves original bytes; failure after persistence returns a
  committed effect with failed finalization, never a plain no-effect error.

- [ ] Two independent sessions in one process do not share progress, directory-sync lists or
...
- [ ] Introduce CapturedInput, PreparedChangeSet and ApplyReport with explicit per-path effects:
...
- [ ] Precompute all deterministic transformations, selector checks, schema checks and output
...
- [ ] Deduplicate physical mutation targets and reject incompatible alias/hard-link selections.
...
- [ ] Implement a CLI-owned apply coordinator using this block operations. Start with serial
  commit for transparent ordering; parallel planning is acceptable. Define stop-on-error and
  not-attempted states explicitly.

- [ ] On every exit after a committed effect, reconcile the current journal with actual changed
...
- [ ] Migrate set/append/task handlers and delete their write-then-early-return paths. Use an
...
- [ ] Two-file append with [] then mapping and task toggle with valid then invalid task line
  perform no writes when the defect is knowable in preflight.

- [ ] Injected second-file I/O failure reports the first committed path and remaining
  unattempted paths; index state is accounted for and a retry cannot be mistaken for a first
  attempt.

- [ ] Injected post-persist permission/finalization failure reports committed bytes accurately.

- [ ] CRLF, inline YAML comments, unrelated properties/body bytes and no-op behavior remain
...
- [ ] Change internal successful payloads from valid-JSON-by-comment String to
...
- [ ] Return typed UserDiagnostic and ExecutionReport values with domain status,
...
- [ ] Give OutputPlan and one renderer responsibility for JSON/text/GitHub output, filename
...
- [ ] Preserve success JSON shape, ordering, optional omission versus null and singular error
...
- [ ] Compile jq before dispatch; if evaluation or rendering fails after writes, preserve the
...
- [ ] Remove obsolete success-string helpers and migrate all command result constructors
  mechanically, keeping command-specific formatting behavior under parity tests.

Acceptance for this stage (focused checks):

- [ ] Every success payload reaches serialization once; existing valid JSON/text/byte contracts
  match the captured baseline except listed fixes.

- [ ] Partial apply and post-write jq error preserve effects in structured stderr and npm
  HyaloError; successful diagnostics remain separate from stdout.

- [ ] Zero-result filename projections produce zero bytes, and invalid user inputs are
  consistently structured when JSON was selected.

- [ ] TS generation and npm contract tests validate old success consumers plus new error
