...
- [ ] LINT-1 (BUG-4): `markdownlint-disable-next-line` suppresses the **following** line
...
- [ ] LINT-2 (BUG-5): `BodySpans` recognises fenced code blocks indented inside list items
...
- [ ] LINT-3 (BUG-43): an unknown rule id or alias in a suppression comment produces a
...
- [ ] LINT-4 (BUG-19): `--max-per-rule 0` means unlimited, as `lint --help` says and
      `--limit 0` already does.
- [ ] LINT-5 (UX-7): decide-or-implement a per-rule option surface for MD010's `code_blocks`
...
- [ ] RECIPE-1 (BUG-6): every mutating `--jq` example in `templates/skill-hyalo.md`,
      ...
      so.
- [ ] RECIPE-2 (UX-4): the `hints` recipe at `skill-hyalo.md:251` cannot work because `--jq`
...
- [ ] RECIPE-3 (UX-5): the broken-links recipe in `.claude/CLAUDE.md` and the templates prints
      `#\(.fragment // "")` after the target so anchor-only breaks are readable.

## SCHEMA — typos must not validate nothing (BUG-20, 42; G5)

- [ ] SCHEMA-1 (BUG-20): `[schema]`, `[schema.types.<t>]` and the wrong nesting
...
- [ ] SCHEMA-2 (BUG-42): `required = ["title"]` means present and non-empty, not `string`;
      ...
      `types --help`.
- [ ] SCHEMA-3 (G5, BUG-41): `set --help` documents the scalar coercion table (`true`,
...
- [ ] INDEX-1 (BUG-11): `--index-file <unreadable>` is an exit-1 JSON envelope like
      ...
      `-q`-proof.
- [ ] INDEX-2 (BUG-12, G4): the snapshot header carries a format version; a snapshot whose
      ...
      constant.
- [ ] INDEX-3 (BUG-30): DEC-302's text says the blind spot is up to ~2 s (whole-second
...
- [ ] PATH-1 (BUG-21): when the CWD is inside the configured vault and `<cwd>/<path>` exists
      ...
      flag (DEC-304 stays).
- [ ] PATH-2 (BUG-22): `[links] case_insensitive = "false"` returns the canonical on-disk
...
- [ ] PATH-3 (BUG-28): the `--dir is redundant` note fires only when `.hyalo.toml` actually
...
- [ ] WRITE-1 (BUG-33): `set`/`append`/`remove` leave a closing fence with trailing
      whitespace byte-identical; the inserted line goes above it.
- [ ] WRITE-2 (BUG-34): decide the opener: either `--- ` (trailing whitespace) opens
...
- [ ] WRITE-3 (BUG-35): `set`/`append`/`remove`/`properties rename` on an unparsable file put
      the YAML diagnostic in the envelope's `cause` and print no bare `error:` line under
      `--format json`.
- [ ] WRITE-4 (BUG-38): `tags rename` keeps a flow-style `tags: [..]` in flow style, using the
      same emitter as `set --tag`.
- [ ] WRITE-5 (BUG-40): `1. [ ]` ordered-list tasks and `-  [ ]` (two spaces) are tasks for
      `--fields tasks`, `summary` and `task toggle`; `- [ ]no space` stays as it is.
- [ ] WRITE-6 (UX-1): bulk `set`/`append`/`remove` envelopes carry `skip_reason` per skipped
      file (`unchanged`, `unparsable`, `schema`, `list_collapsed`), so a same-value write is
      distinguishable from a refusal.
- [ ] WRITE-7 (BUG-44): the `summary` near-duplicate-value warning requires a real string
...
- [ ] HELP-1 (BUG-27): `lint-rules show` prints no `=> lint-rules set` hint for a
      non-configurable rule (SCHEMA).
- [ ] HELP-2 (BUG-29): `hyalo --help` line 35 / 648 and `lint --help` line 179 describe exit 2
      as "usage (clap) or internal error" per DEC-307.
- [ ] HELP-3 (UX-3): `mv --help` says in one sentence why `--on-conflict` offers `error` and
      `skip` but not `overwrite`.

## Shared closing tasks

- [ ] Changelog entries via `hyalo changelog add` (one per group, listing the items).
- [ ] DECs in [[decision-log]]: snapshot format version (INDEX-2), required-vs-string
...
- [ ] Help texts, `rule-knowledgebase.md`, `skill-hyalo.md`, `.claude/CLAUDE.md` and the
...
- [ ] `nl.md` and the list-indented-fence fixtures are byte-identical under `lint --fix`
      except for the lines that must change; the GitHub Docs line-224 proposal is gone.
- [ ] `check-jq-recipes` fails on a mutating recipe without `--dry-run`, and the shipped
      documents pass it; pasting any shipped recipe verbatim writes nothing.
- [ ] `requried = [...]` is a malformed config; `title: 2024` passes a required-only schema.
- [ ] `--index-file /nope` exits 1 with an envelope; the Sep 3 MDN index is refused with the
      version warning; `hyalo config` shows the snapshot version.
- [ ] From `kb/sub/` with `kb/a.md` and `kb/sub/a.md`, `set a.md` warns and names both files.
- [ ] `set` on a file closing with `--- ` changes only the inserted line; JSON-mode errors on
