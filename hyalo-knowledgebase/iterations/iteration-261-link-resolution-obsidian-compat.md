...
- [ ] hyalo-core link parser: treat any target matching `^[a-zA-Z][a-zA-Z0-9+.-]*:`
...
- [ ] Unit tests in hyalo-core for `obsidian://show-plugin?id=x`, `mailto:a@b`,
      `[text](file:///x)`, `<obsidian://open?vault=v>` autolinks, and the
      drive-letter negative.
- [ ] e2e in `crates/hyalo-cli/tests/e2e`: a vault with one `obsidian://` link
      yields `links.broken == 0` in `summary`, no HYALO006, and
      `links fix` lists nothing under `unfixable`.
- [ ] Docs: `find --help` link section, `.claude/skills/hyalo/SKILL.md` broken-link
...
- [ ] hyalo-core file index: index every vault file, not only `*.md`, keyed by
...
- [ ] Classify a resolved non-`.md` target as `attachment` in the link record
...
- [ ] `links fix`: never fuzzy-match across an explicit extension. A broken
...
- [ ] `links fix` and `mv` rewrite the link keeping the `\|` form byte-for-byte,
...
- [ ] DEC-267 (tentative): case-insensitive resolution becomes the default for
...
- [ ] Additive JSON: every entry in `--fields links` carries
...
- [ ] DEC-268 (tentative): the own-KB `[[decision-log#DEC-068]]` anchors (10
      ...
      [[decision-log]].
- [ ] Implement whichever wins, with a unit test for the unique-prefix and the
...
- [ ] Obsidian Hub, cwd `../obsidian-hub`: `hyalo summary --format json --jq
...
- [ ] `../obsidian-hub`: `hyalo find --file "00 - Contribute to the Obsidian
      ...
      kind `attachment`.
- [ ] `../obsidian-hub`: `hyalo find --file "04 - Guides, Workflows, &
...
- [ ] `../obsidian-hub`: `hyalo lint --rule HYALO006 --count` drops from 2897 to
...
- [ ] kepano-obsidian, cwd `../kepano-obsidian`: `hyalo find --broken-links
...
- [ ] Own KB: `hyalo find --broken-links --count` is unchanged or lower, and the
      DEC-068-style anchors are handled per DEC-268.
- [ ] `hyalo find --fields links --format json --limit 1 --jq
...
- [ ] Changelog entry added with `hyalo changelog add`; DEC-266, DEC-267 and
