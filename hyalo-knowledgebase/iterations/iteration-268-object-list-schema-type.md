...
Semantics (DEC-287 — the plan reserved 286, but iteration 267 landed first and took it):
...
- [x] `crates/hyalo-core/src/schema.rs`: add `PropertyConstraint::ObjectList { required_keys:
...
- [x] `RawPropertyConstraint` (line ~499, `deny_unknown_fields`): add `#[serde(rename =
...
- [x] `TryFrom<RawPropertyConstraint>` (line ~515): add the `"object-list"` arm; reject
      ...
      (line ~687).
- [x] Compile every `key-patterns` regex once at config load and fail the config with
      ...
      asymmetry in DEC-287 and leave `item_pattern` as is. Implementation: validate with
...
- [x] `merged_schema_for_type` (line ~258) and `from_raw_lossy` need no change; add a test that a
...
- [x] `crates/hyalo-mdlint/src/schema.rs` `validate_constraint` (line ~868): add the
...
- [x] Severity is `Error`, like every other constraint violation in the `SCHEMA` group
...
- [x] `--fix`: no fixer. `apply_fixes` (line ~423) is untouched. Fix the existing misreport
      ...
      `schema/constraint-violation`. Record the choice in DEC-287.
- [x] `validate_constraint_simple` (line ~1174) needs no change; confirm it returns the first
      object-list message.
- [x] `crates/hyalo-cli/src/commands/new.rs` line ~275: the exhaustive match on
      `PropertyConstraint` must scaffold an object-list property as an empty list `[]`, same as
      `List | StringList`.

### SCHEMA-3: `types show` rendering

- [x] `crates/hyalo-cli/src/commands/types.rs` `constraint_to_json` (line ~87): emit
...
- [x] `crates/hyalo-cli/src/output/text_types.rs` `format_type_show_text` (line ~26): today it
...
- [x] `hyalo config` does not echo schema types (`crates/hyalo-cli/src/commands/config.rs` only
      reports `exempt`), so nothing to do there; note it in the plan so nobody looks.
- [x] `types set --property-type` help (`crates/hyalo-cli/src/cli/args.rs` line ~2495) lists 8
...
- [x] `set --validate` / `validate_on_write` (`crates/hyalo-cli/src/commands/set.rs` line ~481)
...
- [x] Do not add any `set`/`append` syntax for object items. Document in DEC-287 that authoring
...
- [x] Unit, `crates/hyalo-core/src/schema.rs` tests module (next to
...
- [x] Unit, `crates/hyalo-cli/src/commands/lint/tests.rs` (next to
...
- [x] e2e, `crates/hyalo-cli/tests/e2e/lint.rs` (next to
      ...
      `autofixable: false`.
- [x] e2e, `crates/hyalo-cli/tests/e2e/types.rs` (next to `types_show_existing_type`, line
...
- [x] e2e, dot-path interaction (same vault, `lint.rs` or `find.rs`): `hyalo find --property
...
- [x] Config-load e2e: an invalid `key-patterns` regex makes `hyalo lint` report
...
- [x] Help texts: `lint --help` (`args.rs` lines ~1693–1697, the schema-extensions paragraph),
...
- [x] `crates/hyalo-cli/templates/skill-hyalo.md` line ~474 (symlinked as
...
- [x] `docs/configuration.md` line ~324: add `object-list` to the type list.
...
- [x] `hyalo changelog add` under Added: "Schema: `object-list` property type with
...
- [x] DEC-287 in [[decision-log]]: object-list semantics (flat, config-only, no `types set`
      ...
      sugar; nested types).
- [x] Backlog: tick the acceptance boxes in [[backlog/done/object-list-schema-type]] as they land,
...
- [x] Scratch vault with the Goal's TOML (type `memory`, files bound by `type: memory`) and four
      ...
      fixes.
- [x] Same vault: `hyalo types show memory --format json --jq
...
- [x] Same vault: `hyalo find --property sources.ref=github:comparis/neon --format json --jq
      '[.results[].file]'` → `["valid.md"]` (the string-item file never matches, and lint is
      what reports it).
- [x] Same vault: `hyalo set string-item.md -p 'sources=[x]' --validate` exits 1 with the
      object-list message; without `--validate` it writes and the JSON result carries a `note`.
- [x] Vault whose `key-patterns.commit` is `[` : `hyalo lint --strict --count` reports a
      ...
      the schema does not).
- [x] `cargo test --workspace -q`: every existing `list` / `string-list` / `item_pattern` test
      still passes unchanged.
- [x] `hyalo types set memory --property-type sources=object-list` is rejected with the same
...
- [x] Gates green: `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`,
      ...
      check-help-drift`.
- [x] Changelog entries added with `hyalo changelog add`; DEC-287 recorded in [[decision-log]];
