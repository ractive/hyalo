...
- [ ] **Property↔dirname coupling rule**: property value must equal parent directory name (generic: `equals = "$parent-dir"` or a dedicated lint rule) — needed for `name`. Exclude the file itself when scanning; add a self-reference/degenerate-case regression test (see iter-167 PR-review retrospective above)
