...
- [ ] Introduce DocumentFrame/FrontmatterFrame carrying exact byte spans, body offset, newline
  style, BOM policy and framing diagnostics; keep parsed property values separate from source
  spans.

- [ ] Use bounded incremental BufRead filling instead of allocating full lines before checking
  limits. Share byte/line/node/depth budgets with the normal YAML parser and define boundary
  equality behavior.

- [ ] Replace the scanner fixed 16-KiB metadata prefix, lint substring delimiter and duplicated
...
- [ ] Validate the final serialized/spliced frontmatter using the same reader budget before
  committing. Remove higher-budget verification that permits files the normal reader rejects.

- [ ] Migrate read, metadata scans and lint body slicing first, then all frontmatter writer
  entrypoints. Keep source offsets compatible with existing line numbering and byte-preserving
  splices.

- [ ] Add generated boundary fixtures and a counted reader that demonstrates allocation/read
  caps before adversarial long lines are materialized.

Acceptance for this stage (focused checks):

- [ ] Empty, missing and malformed delimiters, ---note keys, CRLF, BOM, trailing delimiter
  whitespace and EOF are interpreted identically across consumers.

- [ ] Valid 20-KiB frontmatter is accepted by both metadata-only and full scans; exact limit and
  limit-plus-one cases have consistent diagnostics.

- [ ] Every successful writer output is readable under the normal parser budgets; a
  near-node-limit list refuses an overflowing edit before mutation.

- [ ] Metadata-only scans do not read the body; memory is bounded independently of first-line
  length.
...
- [ ] Extract or extend a core BodySyntax/SpanCursor API with source offsets and classifications
...
- [ ] Make structural visitors inspect classified visible syntax and use authored raw bytes only
...
- [ ] Move the shared classification below hyalo-mdlint so dependency direction remains mdlint
  -> core. Keep rule configuration/severity/profile behavior in mdlint; do not move third-party
  lint dependencies into core.

- [ ] Migrate native HYALO rules and directive recognition onto spans. Recognize disable
  directives only in syntactic HTML comments; preserve existing fence/container and
  disable-next-line semantics.

- [ ] Use shared ATX heading parsing and a section-filter type that accepts its documented
  Unicode input without debug-only assumptions. Retain source offset/newline mappings for fixes.

- [ ] Delete or adapt the superseded scanner BodyState/LineScanner/BodySpans implementations;
  document the one remaining policy authority and consumers.

Acceptance for this stage (focused checks):

- [ ] Code blocks, inline code and HTML comment examples remain byte-identical under lint --fix
  unless a rule explicitly targets those regions.

- [ ] Hidden comment headings/tasks do not appear structurally; full-text queries still find
  literal content when requested.

- [ ] ATX closing hashes, C#, escaped hashes, CRLF and Unicode section selectors behave
  consistently in debug/release and index/disk paths.

- [ ] Existing stock-rule and native-rule suppression cases pass through the shared
...
- [ ] S05: valid ---note: "a<TAB>b" inside YAML must retain the literal tab after lint --fix
  --rule MD010. Include delimiter-like keys, empty frames and supported closing-line whitespace.

- [ ] S09: a flow list with 4,997 scalar items near the 5,000-node budget must not become
...
- [ ] S11: use a counted/generated reader with a huge first line and a huge line after the
...
- [ ] S13/R05: compare valid 20-KiB metadata-only find against full-body scans, direct read and
  indexed scans. Test 16-KiB split points inside UTF-8 and around the closing delimiter.

- [ ] R11: description: | followed by an indented # `[[Target]]` is scalar content. Track
...
- [ ] Run the existing frontmatter-splice/scanner fuzz seeds and add these compact boundary
  cases; keep writer preservation and resource limits in the same test oracle.

Acceptance for this stage (focused checks):

- [ ] Metadata-only and full scans agree on accepted frontmatter and diagnostics without
  requiring body reads for metadata-only results.

- [ ] Every successful write is readable under standard limits and preserves unrelated body/YAML
  bytes.

- [ ] Block-scalar links appear in backlinks, broken-link checks and rewrite source mapping;
  YAML comments do not create false links.

- [ ] Bounded-reader tests use small controlled resource budgets rather than deliberately
  exhausting the host.
...
- [ ] Literal regions remain unchanged under native/stock lint fixes; hidden structure stays
  absent while full-text literal search remains supported.

- [ ] Directive parsing honors syntax and existing suppression scope without disabling rules
  from examples.

- [ ] Heading/task behavior agrees in standalone and indexed consumers on LF/CRLF and Unicode
  fixtures.

- [ ] Type-scoped and explicitly targeted lint agree for valid numbered filenames; zero checked
