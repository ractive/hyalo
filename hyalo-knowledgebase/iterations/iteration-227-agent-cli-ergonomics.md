---
title: "Iteration 227 — agent-facing CLI ergonomics: find --filenames-only, --iteration addressing, boundary-error hints"
type: iteration
date: 2026-08-24
status: planned
branch: iter-227/agent-cli-ergonomics
tags:
  - iteration
  - cli
  - agent-ergonomics
...
# Iteration 227 — agent-facing CLI ergonomics: find --filenames-only, --iteration addressing, boundary-error hints
...
- [ ] Simplify ralph-loop skill to consume the new flags
      (`~/.pi/agent/skills/ralph-loop/scripts/preflight.sh` `discover_plan()`
      can drop its glob+fallback dance once `find --iteration` exists; keep
      the shell-glob fallback for vaults without template config) — do this
      after the CLI work merges, as a follow-up commit in this same iteration
      (skill lives outside the repo; the commit only documents the change in
      the knowledgebase)
...
- [ ] The ralph-loop skill's three hyalo-usage warnings are obsolete: with
      `--filenames-only` and `--iteration`, none of the warned-against
      patterns are needed (verify by porting preflight's `discover_plan`)
...
- [ ] No behavior change for existing invocations: no new flag → identical
      output byte-for-byte (regression-tested on a snapshot of current
      find/set output in the e2e suite)
