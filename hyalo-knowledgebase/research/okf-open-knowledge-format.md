...
3. **No OKF init profile.** `init` is hardcoded to Claude/pi skills (`crates/hyalo-cli/src/commands/init.rs`); no path to scaffold an OKF-shaped `.hyalo.toml`. NB: the flag must be `--profile okf`, not `--format=okf` — `--format` is the global output flag and collides. **DONE (iter-164):** `hyalo init --profile okf` deep-merges an embedded declarative TOML fragment (`templates/profile-okf.toml`) into `.hyalo.toml` — `required=["type"]`, recommended props (`timestamp:datetime-tz`, `resource` URL pattern), `exempt=["**/index.md","**/log.md"]`, `site_prefix=""`, `validate_on_write=true`, seeded `[schema.types.*]`. Profiles are data-driven (`crates/hyalo-cli/src/commands/profiles.rs`), composable (deep-merge upserts only their own keys), and idempotent on re-run. [[iteration-164-okf-init-profile-and-skill]].
4. **No `index.md` / `log.md` generators.** These are *derived* data; maintaining them by hand is exactly the tedium hyalo should kill. `hyalo okf index` / `hyalo okf log` have no equivalent anywhere in the OKF ecosystem — highest-leverage, unique. **DONE (iter-165):** `hyalo okf index` regenerates every directory's `index.md` from concept frontmatter — grouped by `type` (untyped → `Other`), `* [title](relative-link) - description` lines (title falls back to filename stem), immediate subdirectories listed, generated list confined to a stable managed region (`<!-- okf:index:begin -->` / `<!-- okf:index:end -->`) so hand-written prose is preserved, bundle-root `okf_version` kept, relative forward-slashed links, idempotent, `--dry-run` (default) exits non-zero on drift for CI. `hyalo okf log [TARGET]` prepends a dated `YYYY-MM-DD` entry (newest first) to a scope-selectable `log.md` (directory-local per §7; directory / `log.md` path / omitted → bundle root), optional `--action` bold word, creates the file frontmatter-free, validates `TARGET` stays in-bundle. Both dry-run by default, mutate with `--apply`. Also landed: `normalize_key_order` (canonical `type, resource, title, description, tags, timestamp`) and a `now_timestamp_tz()` `+00:00` helper (`crates/hyalo-cli/src/commands/okf.rs`). [[iteration-165-okf-index-and-log-generators]].
5. **No conformance profile.** Spec §9 = 3 rules; no `hyalo lint` ruleset encodes it, and broken-link checks must be *warn* not *error* to stay spec-compliant. **DONE (iter-166):** `hyalo lint --profile okf` is an ephemeral overlay that merges the *same* `profile-okf.toml` fragment `init --profile okf` materializes (via the shared `profiles::merge_into_config`) then re-parses it — so it needs no config (CI / third-party bundles) and is idempotent: on a vault initialized with the profile (which the fragment now stamps with `[lint] profile = "okf"`) a plain `hyalo lint` runs the same rules. SPEC §9 errors only on missing frontmatter / empty-or-missing `type` (reserved `index.md`/`log.md` stay exempt); everything else is **warn, never reject** — broken cross-links, reserved-file structure (`index.md` link-list §6, `log.md` date-grouped-newest-first §7), citations, and augmentation regressions. Unknown `type`/extra keys always pass. Six advisory rules (`OKF-INDEX-STRUCTURE`, `OKF-LOG-STRUCTURE`, `OKF-CITATIONS-PRESENT/-WELL-FORMED/-RESOLVE`, `OKF-AUGMENTATION-GUARD`) live in `crates/hyalo-cli/src/commands/okf_lint.rs`, appear in `lint-rules list`, and are individually toggleable; `--profile` composes with `--fix`/`--rule`/`--strict`/`--files-from` (no forked path). [[iteration-166-okf-conformance-lint]].
6. **No bundled OKF skill.** `init --claude` installs `hyalo`/`hyalo-tidy` skills; no OKF-authoring/producer skill. **DONE (iter-164):** `templates/skill-hyalo-okf.md` embedded in the binary; `hyalo init --profile okf --claude` installs it at `.claude/skills/okf/SKILL.md`. Teaches the OKF concept model, reserved-file rules, link forms, the deterministic-vs-LLM split, and only real, current hyalo commands (`find --property type=`, `new --type`, `lint --strict`, `set`/`append`, `mv`, `find --broken-links`) — deferring `okf index`/`log`/`lint --profile` to iters 165–166. [[iteration-164-okf-init-profile-and-skill]].
7. **No citation-aware linting.** hyalo has only generic markdown link rules (MD011/034/042/…) + internal broken-link repair (`hyalo links`); nothing understands the `# Citations` convention. Missing: *presence* (warn when a claim-bearing concept has no `# Citations`), *resolvability* (bundle-relative / `references/` citation links resolve), *non-regression* (don't shrink a doc's citation count — the reference-agent's own augmentation guard). External URL reachability stays out (determinism/offline). A convention (SHOULD), so warn-level advisory in the okf profile, not §9 conformance. Nuance: §8 says citations are *numbered*, but all official sample bundles use `-` bullets — accept both. **DONE (iter-166):** shipped as `OKF-CITATIONS-PRESENT` (claim-bearing = non-reserved, non-`Reference` concept), `OKF-CITATIONS-WELL-FORMED` (entries are links, not prose; numbered *and* bullet lists accepted), `OKF-CITATIONS-RESOLVE` (bundle-relative/`references/` paths stat'd on disk, external URLs surfaced-not-fetched), plus `OKF-AUGMENTATION-GUARD` as the baseline-free non-regression proxy (warns when a `# Schema`/`# Citations` section is present-but-empty). All warn-level, behind the okf profile, individually toggleable via `lint-rules set`. [[iteration-166-okf-conformance-lint]].
8. **Bundle-absolute link edge case.** SPEC §5 *recommends* leading-`/` bundle-root links ("stable when documents are moved"). `strip_site_prefix` (`crates/hyalo-core/src/link_graph.rs:569`) already falls back to vault-root resolution on prefix mismatch, so these mostly work today — but the auto-derived `site_prefix` (vault dirname) mis-strips when the bundle dir shares a name with a top-level subdir (bundle root named `tables/` breaks `/tables/x.md`). The OKF profile must pin `site_prefix`; `--site-prefix ""` currently means "disable resolution", which is also wrong for OKF. **DONE (iter-163):** confirmed `site_prefix = ""` (→ `None`) is exactly right — it strips only the leading `/`, so `/tables/x.md` resolves to `tables/x.md` from bundle root and the dirname-collision mis-strip is avoided (an active prefix matching a subdir name would break it). Documented + tested; the OKF init profile (iter-164) sets `site_prefix = ""`. See [[iteration-163-okf-frontmatter-foundations]].
...
3a. **No `okf lint` (considered 2026-07-16).** Validation stays centralized in `hyalo lint`: its flag surface (`--fix`, `--rule`, `--strict`, `--files-from -`) must compose with profiles, per-profile lint subcommands would fork it ×N profiles, and the `okf` group exists only for OKF-specific *artifact generators* — validation is the shared engine with a different ruleset. Semantics: `lint --profile okf` is the *ephemeral overlay* (CI, third-party bundles); after `init --profile okf` the rules live in `.hyalo.toml` and **plain `hyalo lint` suffices**. Discoverability via hints: `hyalo okf` commands emit `-> hyalo lint --profile okf`.
...
6. **Per-profile command group, not a generic profile-keyed generator (iter-167, 2026-07-17).** `hyalo madr toc` follows the same `hyalo okf index` shape rather than a single `hyalo generate <profile>` dispatcher: each profile's generator has its own semantics (index vs TOC, different frontmatter fields, different managed-region prefix), so a generic wrapper would immediately need per-profile branches anyway — better to keep the shared *mechanics* (`commands/managed_region.rs`'s `Markers`/`GeneratePlan`/`apply_plan`, dry-run-exits-nonzero-on-drift) factored out and let each profile own a thin, self-documenting subcommand group. Iter-169's `changelog release` is expected to follow the same pattern.
