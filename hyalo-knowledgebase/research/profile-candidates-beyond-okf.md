...
3. **`skills`** — [[iteration-168-skills-profile]] ✅ **SHIPPED (2026-07-17)** — added a generic **string length bound** (`min-length`/`max-length`) on any `string` property (the first cross-profile new capability), plus three advisory rules gated on `--profile skills`: **`SKILL-RESERVED-NAME`** (error — `name` ∈ {`anthropic`, `claude`}, since the slug regex cannot express negation without look-around), **`SKILL-NAME-DIRNAME`** (warn — `name` == parent dir), **`SKILL-LINE-BUDGET`** (warn — body <500 lines). The `skill` type is path-bound to `**/SKILL.md`; `name`/`description` regex+length are hard schema errors; `user_invocable` typed as a Claude extension so this repo's `.claude/skills/` lints clean. Decisions: reserved-word + name↔dirname checks live in an advisory rule (not the schema pattern) because Rust's `regex` crate has no look-around and the dirname needs the file path; description "no XML tags" enforced as `pattern = "^[^<]*$"` (lookahead-free). Scaffolding reuses the generic `hyalo new --type skill --file <name>/SKILL.md` — no bespoke auto-numbering was needed.
4. **`changelog`** — [[iteration-169-changelog-profile]] ✅ **SHIPPED (2026-07-17)** — generalized the **heading-grammar lint mode** into a reusable declarative engine (`heading_grammar.rs`: a CRLF/fence-aware scanner + `HeadingRule` level/text matchers with per-rule severity), the genuinely new capability of the sequence. The `changelog` type is path-bound to the literal `CHANGELOG.md` and exempt from the frontmatter rules (frontmatter-less body grammar). Eight `CHANGELOG-*` rules — mostly **error** (title, version/category heading shape, semver-descending order, date monotonicity, `[Unreleased]` pinned first), two **warn** (empty section, footer link-ref cross-check). Added `hyalo changelog release <X.Y.Z>` (rotate `[Unreleased]` → dated section, recreate empty Unreleased, append `[X.Y.Z]: TBD` link ref, refuse duplicate) and `hyalo changelog add --category`, both dry-run-by-default + drift exit code. Decisions: `changelog add` **earned its keep** (makes the release round-trip authorable end to end and mirrors `okf log`'s dated-insert shape); the release link-ref uses a `TBD` placeholder because compare URLs are repo-specific; footer link-def lines are *not* counted as section content (so a released section with only a trailing footer ref still flags empty). **Not adopted in this repo:** hyalo's own `CHANGELOG.md` uses `## X.Y.Z — DATE` (em-dash, no brackets, no `[Unreleased]`, no footer refs) — it is not a Keep a Changelog file, so the profile was not turned on here.
...
1. **Heading-grammar lint mode** (frontmatter-less structure specs) — changelog, standard-readme, Nygard ADR, OKF `index.md`/`log.md` structure checks. ✅ **DELIVERED (iter-169)** as `heading_grammar.rs`: a declarative `HeadingRule` (level + `TextMatcher::{Exact,OneOf,Predicate}` + per-rule severity) over a CRLF/fence-aware scanner. changelog is its first consumer; Nygard ADR and standard-readme can now declare a grammar instead of re-scanning. *Not yet generalized:* cross-heading **sequence/ordering** constraints (changelog's semver-descending + `[Unreleased]`-pinned checks still live in `changelog_lint.rs`, not the engine) — lift those next when a second grammar consumer needs them.
...
## Consolidated profiles retrospective (2026-07-17, sequence complete)

Four profiles shipped in four iterations (okf → madr → skills → changelog). What the profile machinery got right and wrong across the sequence:

**What the machinery got right.** The core bet — *profiles as embedded declarative TOML fragments, deep-merged into `.hyalo.toml`, additive by a single `Profile` entry* — held for all four. Adding a profile never required a new match arm in dispatch/run; it is one `PROFILES` entry + a `templates/profile-<name>.toml` + a skill file. The **ephemeral-overlay** (`hyalo lint --profile <name>` reuses the same merge in-memory) and **`[lint] profile`** idempotence machinery from iter-164 carried through verbatim; every profile lints clean on a bare directory with no committed config (CI/third-party scenario). **Composability** works: okf + madr + skills + changelog coexist in one vault (verified by test), because the merge upserts only its own keys and `[[schema.bind]]` is first-match-wins.

**What stayed stubbornly non-data-driven.** Advisory-rule *activation* is still a **hardcoded boolean per profile** (`<name>_profile_active` in `dispatch.rs` → `ExtLintOptions.<name>_profile` → a fn param → an `if <name>_profile { … }` block in `lint.rs`). Four profiles = four near-identical copy-paste blocks. This is the one place adding a profile touches Rust in more than one file. **Recommendation for the next profile wave:** replace the four booleans + four blocks with a single registry — a `&[ProfileRuleSet]` keyed by profile name, each carrying its `run_*_rules` fn pointer — so activation becomes data. Deferred here only because the fifth profile would be the point where the abstraction pays for itself (rule of three passed at skills; do it before the SSG wave).

**Per-rule severity was the recurring surprise.** okf assumed all-warn; skills needed one error (`SKILL-RESERVED-NAME`); changelog is mostly-error with two warns. The `default_severity: &'static str` per finding (added in skills, adopted from the start in changelog) is the right shape — but the lesson (repeatedly caught in PR review) is that **any doc comment claiming "these rules are advisory/warn" goes stale**; name the exceptions or point at the per-rule field. changelog's engine and catalog comments do the latter.

**Rust `regex` has no look-around** bit skills (reserved-word negation, "no XML tags") and constrained changelog (semver/date patterns kept lookahead-free; ordering logic lives in Rust, not a `pattern`). This is now a known constraint to design around, not rediscover.

**Generic capabilities delivered, in order:** `[[schema.bind]]` path-bound schemas (madr, reused by skills + changelog), `{n:04}` filename tokens (madr — still not wired end-to-end, only madr needs it), string `min-length`/`max-length` (skills, available to all), and the `heading_grammar` engine (changelog). Each new profile contributed exactly one reusable mechanism — a healthy sign the sequence was scoped well.

### Revised effort estimates for deferred candidates

- **Nygard / adr-tools ADR** (headings-only, no frontmatter): **small (0.5 iteration)** now that `heading_grammar` exists — it is a grammar declaration + a path binding + a skill, no new engine. Best next profile: it exercises the grammar engine a second time and would force the sequence/ordering generalization noted in gap #1.
- **Standard Readme**: **small–medium (0.75 iteration)** — heading grammar for the required-section sequence (`# Title` → badges → sections in order) + a generator for the TOC. Mostly grammar + one managed-region generator (reuse `managed_region.rs`).
- **SSG wave** (`hugo`/`docusaurus`/`jekyll`/`mkdocs-material`/`starlight`): **medium each, but front-loaded** — the first one costs ~1.5 iterations because it needs **enum-from-external-file** (gap #3) and **deprecated-key→replacement lint** (gap #7); subsequent SSGs are ~0.75 once those land. Hugo additionally blocks on **TOML `+++` frontmatter** (gap #9). Recommend doing docusaurus or jekyll first (clean YAML, no TOML gate).
- **Importers** (`decap`/`frontmatter`/`obsidian`/`types.json`): **medium (1 iteration for the framework + 1 per source)** — orthogonal to profiles (they *generate* a fragment rather than ship one). The `merge_into_config` deep-merge is already the write target. Lower priority: one-way, modest payoff.
- **Do the rule-activation registry refactor first** (above) — **small (0.5 iteration)**, pays down the one piece of per-profile Rust before the wave multiplies it.
