...
- **L-21** RESOLVED in [[iterations/iteration-190-link-anchors]] (2026-07-19) — anchors now carry through resolution (`Link.fragment`) and `find --broken-links` reports a distinct `broken_anchor` category (target resolves, `#heading` missing), never conflated with a broken target and never inflating `links fix` counts. Exact case-insensitive heading match (`hyalo-core/src/anchor.rs`), percent-decoded markdown fragments, `^block-id` refs skipped, zero extra file reads (validated against indexed/scanned `sections`). The `fragment` field proved backward-compatible under `to_vec_named` framing (old snapshots decode with `fragment: None`; rebuild to pick up anchor data) — see DEC-060 / DEC-061. Fragment-only links remain correctly non-file-links.
