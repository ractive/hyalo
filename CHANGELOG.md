...
- The *literal* half of link classification — "is this link already correct exactly as written?", asked once per link before any case-folding — now answers from that same file set instead of a `stat` plus a `realpath` per candidate path (DEC-322). It was the last un-indexed probe and, on a copy of MDN with `--site-prefix en-US/docs`, 95 % of an indexed `summary`: 2.01 s → **0.26 s**, with `summary`, `find --broken-links` and `links fix --dry-run` output byte-identical on the disk scan and on `--index` alike. `links fix` over the 14 000-file scale corpus went from ~3.4 s to 1.08 s. A `--file`-scoped run, whose index proves nothing about the rest of the vault, still probes the filesystem.
